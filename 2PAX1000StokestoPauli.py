# -*- coding: utf-8 -*-
"""
Created on Mon Feb  5 14:40:57 2024
...
# Define function to convert Stokes parameters to Pauli spin matrices
def stokes_to_pauli(S0, S1, S2, S3):
    pauli_matrix = S0 * sigma0 + S1 * sigma1 + S2 * sigma2 + S3 * sigma3
    return pauli_matrix

# Convert Stokes parameters to Pauli spin matrices for the first PAX1000
pauli_matrices1 = []
for i in range(len(S01)):
    pauli_matrix = stokes_to_pauli(S01[i], S11[i], S21[i], S31[i])
    pauli_matrices1.append(pauli_matrix)

# Convert Stokes parameters to Pauli spin matrices for the second PAX1000
pauli_matrices2 = []
for i in range(len(S02)):
    pauli_matrix = stokes_to_pauli(S02[i], S12[i], S22[i], S32[i])
    pauli_matrices2.append(pauli_matrix)

# Save the Pauli spin matrices to CSV files
df1 = pd.DataFrame(pauli_matrices1[0])
df1.to_csv('pauli_measurement1.csv', index=False, header=False)
df2 = pd.DataFrame(pauli_matrices2[0])
df2.to_csv('pauli_measurement2.csv', index=False, header=False)
