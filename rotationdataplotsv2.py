# -*- coding: utf-8 -*-
"""
Created on Wed Jan  8 23:45:58 2025
...
try:
    while True:
        ...
        # Plotting and saving the frame
        plt.clf()  # Clear the current figure
        plt.plot(time_stamps, rotation_rates, label='Rotation Rate', color='blue')
        plt.xlabel('Time [s]')
        plt.ylabel('Rotation Rate (Omega)')
        plt.title('Real-time Rotation Rate Plot')
        plt.legend()
        plt.grid()

        # Save the frame
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        frame_filename = f"rotation_rate_frame_{timestamp}.png"
...
# Final plots and FFT calculations can be done here if needed...
