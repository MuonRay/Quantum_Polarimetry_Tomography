# -*- coding: utf-8 -*-
"""
Created on Thu Nov 23 15:51:30 2023
2 PAX1000 connection with PER calculation 
...
#modify range as needed, for full tomography we need 16 measures
    for x in range(5):
        revolutionCounter = c_int()
        scanID = c_int()
...
# Calculate the PER for the first PAX1000
PER1 = np.sqrt(S11**2 + S21**2 + S31**2) / S01

# Calculate the PER for the second PAX1000
PER2 = np.sqrt(S12**2 + S22**2 + S32**2) / S02

# Save the PER values for the first PAX1000 into a CSV file
with open("PER1.csv", "w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["Measurement", "PER"])
    for i in range(len(azimuth_measures1)):
        writer.writerow([i+1, PER1[i]])

# Save the PER values for the second PAX1000 into a CSV file
with open("PER2.csv", "w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["Measurement", "PER"])
    for i in range(len(azimuth_measures2)):
        writer.writerow([i+1, PER2[i]])
